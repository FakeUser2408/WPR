...
/**
 * Compare site photos between two weeks using Supabase edge function.
 * The backend handles image download and Claude analysis.
 */
export async function compareSitePhotos(
  prevWeekImages: Blob[],
  currWeekImages: Blob[],
  prevWeekNum: number,
  currWeekNum: number,
  ...
  if (!projectName) {
    return {
      ...
      message: "Project name is required for image comparison",
      findings: [],
      recycled_photos_detected: false,
      areas_compared: [],
    };
  }
  ...
  try {
    // Call the Supabase edge function (no auth header - functions are public)
    const response = await fetch(
      `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/compare-site-photos`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!response.ok) {
      ...
      return {
        status: "error",
        message: `AI Vision analysis failed: ${error.error || error.message}`,
        findings: [],
        recycled_photos_detected: false,
        areas_compared: [],
      };
    ...
    const data = await response.json();
    return data?.image_comparison || {
      status: "error",
      message: "No response from image comparison service",
      findings: [],
      recycled_photos_detected: false,
      areas_compared: [],
    };
    ...
    return {
      status: "error",
      message: `AI Vision analysis failed: ${err.message}`,
      findings: [],
      recycled_photos_detected: false,
      areas_compared: [],
    };
