...
const Index = () => {
  ...
  const handleAnalyze = async () => {
    if (!wpr1.text || !wpr2.text) {
      toast({ title: "Missing files", description: "Please upload both WPR PDFs", variant: "destructive" });
      return;
    }
    setIsAnalyzing(true);
    setProgress(10);
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h1 className="text-3xl font-bold">Quick Analysis</h1>
          </div>
          <p className="text-muted-foreground text-lg max-w-2xl">
            Upload two WPR PDFs side-by-side for a one-off comparison. For project tracking, use the <a href="/projects" className="text-primary hover:underline">Projects</a> page.
          </p>
        </motion.div>
